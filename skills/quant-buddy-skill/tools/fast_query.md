...
**不适用**：选股、回测、行业聚合、事件研究、K线、开放式个股指标画像或全维度指标概览（此类走 `stockProfile`）；用户要求画像指标的“上一值 / 前值 / 前后两期对比”时也走 `stockProfile`，由其返回 `previous_value` / `previous_date`，不要用 `fast_query` 临时拼接；期货估值/财务/K线不在本接口承诺范围内。
