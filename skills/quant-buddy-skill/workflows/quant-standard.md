...
5. 读取结果时，**必须只读 TopN 相关数据**：
   - 优先读取 TopN 掩码对应的展示值：`readData(mode="precheck")` 先确认结果规模
...
4. **覆盖率完整性**：`readData(mode="precheck")` 返回的 `total_count` 应与目标资产宇宙规模基本匹配。若全A股排名但 `total_count` 远低于预期（如仅 300 只，而全A应有 5000+），说明资产宇宙收敛可能存在问题（如分钟频数据覆盖不足、板块掩码维度不对齐）——此时必须在答案中**明确披露**实际覆盖范围（如"本次排名基于分钟频数据覆盖的 XXX 只 A 股"），不得将其表述为"全A股排名"
...
若报"函数不存在"：回查案例 `card_id` 原文确认是否有抄写差异，再用 `searchFunctions` 找正确函数名。
