...
| 6 | `readData` | `{"ids": ["hex_id", ...], "mode": "smart_sample"}` — **hex data_id**，最多 10 个 | **不可跳过**：验证 NaN率、净值方向、覆盖率。⚠️ `ids` 必须是 `runMultiFormulaBatch` 返回的 hex `_id`，**不能传中文变量名**（如 `"A股收盘价"`） | mode 不是 smart_sample → **必读 `tools/read_data.md`** |
