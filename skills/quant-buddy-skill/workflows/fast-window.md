...
当 资产 × 字段 × 日期数 > 500 时，服务端通常**按设计**返回 CSV 模式（`mode:"csv"`，每个字段一个 `csv_url`，OSS 链接、有有效期）；港股财报因服务端多来源合并仍返回 JSON。**CSV 链接是大数据量的正常交付路径，不是失败**；但**把链接直接丢给用户不算完成**——除非用户明确只要下载文件。
