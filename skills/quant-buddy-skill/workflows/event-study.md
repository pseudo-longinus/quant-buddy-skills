...
| E3 | 数据已取 | runMultiFormula + readData 完成 | 每个窗口至少有 1 个有效数值；无全 NaN 列 |
...
| E3 runMultiFormula 失败 | E2 | 公式（换 prefix 或简化窗口） | 同时改日期和公式 |
...
| 条件类型 | 示例 | 日期来源 |
|---------|------|---------|
| **可量化阈值** | 沪深300回撤>30%、油价月跌幅>20%、PE<10 | **必须**数据驱动（runMultiFormula + readData） |
...
3. **用 runMultiFormula 在公式层完成阈值筛选**（begin_date 需覆盖足够历史区间）——必须在公式中直接生成布尔掩码（如 `HIT=("月收益率"<-0.20)`），而不是取回完整序列后在回答层人工扫描
...
- 必须让 runMultiFormula 在公式层完成 `<` / `>` 阈值判断，只把 0/1 结果传给 readData
...
> **Step D 是关键**：必须在 runMultiFormula 中完成阈值判断，生成 0/1 掩码。禁止跳过 Step D 直接对 Step C 的连续序列做 readData 全量扫描——序列过长会被截断导致遗漏。
...
> 对"过去N年/历次X事件"类问题，若平台数据可直接识别阈值事件（如通过 runMultiFormula 计算阈值触发日），不得优先使用 webSearch 生成事件池；webSearch 仅作兜底或证据补强。
...
将 Step 2 返回的 `formulas` 数组直接传入 `runMultiFormula`：

```json
runMultiFormula({"formulas": [...Step 2 返回的 formulas...]})
```
