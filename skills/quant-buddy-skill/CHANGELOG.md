...
更新正式库资产、系统数据名和板块预设，版本升至 `4.24.5`。

- **正式资产库同步**：维护脚本默认连接正式库并支持读取 `MONGODB_URI`，查询时排除 `deleted: true` 的软删除资产；期货预设从 257 条调整为 244 条，清理 13 条已软删除记录。
- **系统数据名目录**：从 2,357 条增至 2,358 条，`guanzhao` 新增 `创业板指指数成分股`；导入器期望总数同步为 2,358，清单校验保持 `warnings: []`。
- **板块预设**：从 741 条增至 742 条，新增 `上证50`，名称和顺序与 `板块_all_742.xlsx` 完全一致。
