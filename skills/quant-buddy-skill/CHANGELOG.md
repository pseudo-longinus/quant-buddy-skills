...
- 增加日行情与季度财务按发布日前向填充的标准筛选示例；分别保留用户原文和解析口径。
- 明确 QBS 批次执行证据与 QBV 包级发布合同边界、stdout JSON/stderr 进度分离。
- 历史 fixture 验证 20260914 的 12 只结果，拒绝报告期或最后发布日误充目标交易日；不修改计算引擎。
- 修正旧元数据测试硬编码历史版本号导致新版本误报的问题。
...
- `fast_query(snapshot)` 与 `fast_query(window)` 的资产解析统一为：用户输入全为数字时，先做数字部分完全一致的匹配；只要命中 `stock` 类型资产，即优先返回股票，不因同号指数、基金或其他资产而改选非股票。
- 同号命中多只股票时先确认市场；没有同号股票时保留原有“唯一精确命中才直接返回”的处理。带市场前缀/后缀的完整代码仍严格精确匹配，禁止数字子串模糊命中。
- 补充 snapshot/window 双入口一致性的离线契约测试；本次仅更新 skill 源码，未自动发布或安装到宿主。
...
- 新增市场×资产类型分钟历史起点表；历史请求先检查，全窗口越界不调用，部分覆盖先提示、不静默改日期；单日工具与原生工具描述同步区分暂不支持市场。
...
- 新增 fast_query_minute_range 的原生声明、执行路由、参数校验与专用CSV长表读取器；历史/单日/日频场景明确分流。补充 fast_query、fast_query_minute 的合约/换月/警告返回语义，保留老工具fields。
